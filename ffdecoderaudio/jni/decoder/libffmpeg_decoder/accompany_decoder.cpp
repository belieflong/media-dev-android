...
/**
 * 该方法会调用两次
 * (1).获取音频信息[默认-采样率:44K 通道:2 量化格式:6]，并进行重采样量化格式修改为16
 * (2).解码初始化
 * @param audioFile
 * @return
 */
int AccompanyDecoder::init(const char* audioFile) {
	...
	if(NULL == accompanyFilePath){
        ...
        LOGI("lyl accompanyFilePath = %s", accompanyFilePath);
	}
	...
	if (!audioCodecIsSupported()) {
		 ...
		 */
		LOGI("lyl swr_init front channels=%d sample_fmt=%d sampleRate=%d", avCodecContext->channels, avCodecContext->sample_fmt , avCodecContext->sample_rate);
		// 构建音频的格式转换对象
	...
	pAudioFrame = av_frame_alloc(); // 创建音频解码后数据存放的地方
...
int AccompanyDecoder::readSamples(short* samples, int size) {
	...
	int sampleSize = size;
	while(size > 0){ //可以理解"buffer为剩余可缓存容量"
		if(audioBufferCursor < audioBufferSize){ //游标大小 小于 解码大小，说明解码有新数据了，否则进行解码数据
			int audioBufferDataSize = audioBufferSize - audioBufferCursor;
			int copySize = MIN(size, audioBufferDataSize); //控制size的最小值为0(自己-自己)
			memcpy(samples + (sampleSize - size), audioBuffer + audioBufferCursor, copySize * 2);
			size -= copySize;              //更新剩余大小=初始化大小-拷贝大小
			audioBufferCursor += copySize; //更新游标位置
		} else{
			if(readFrame() < 0){
				break;
			}
		}
		//LOGI("sampleSize is %d , size is %d", sampleSize, size); //size为剩余容量，为0时buffer放满了
	}
	int fillSize = sampleSize - size;//已经填充的大小=初始大小-剩余大小 (大小与packetBufferSize一致即读满了，剩余缓存容量为0)，填充满了以后开始write
	if(fillSize == 0){
        LOGI("readSamples return .......结束解码");
		return -1;
	}
	return fillSize;
}
...
void AccompanyDecoder::seek_frame(){
	...
	while (true) {
		...
		readFrameCode = av_read_frame(avFormatContext, &packet);//该方法取出来的数据是AVPacket
		if (readFrameCode >= 0) {
...
//	LOGI("enter AccompanyDecoder::readFrame");
	...
	int readFrameCode = -1;
	while (true) {
		readFrameCode = av_read_frame(avFormatContext, &packet); // 读取压缩数据 将压缩数据作为解码器的输入
//		LOGI(" av_read_frame ...", duration);
		if (readFrameCode >= 0) {
			if (packet.stream_index == stream_index) {
				int len = avcodec_decode_audio4(avCodecContext, pAudioFrame, // 解码器将其解码为原始数据
						&gotframe, &packet);
